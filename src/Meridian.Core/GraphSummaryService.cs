...
public sealed class GraphSummaryService
{
    ...
    public AgentSummaryResult Summarize(GraphDocument graph, GraphSummaryOptions? options = null)
    {
        ...
        var nonContainmentEdgesByNode = BuildNonContainmentEdgesByNode(graph);
        ...
        var clusters = BuildClusters(graph, nonContainmentEdgesByNode, limit);
    ...
    public static IReadOnlyList<string> BuildLimitations(GraphStatistics statistics, string? clusterLimitation = null)
    {
        ...
        {
            limitations.Add("No endpoint nodes are present in the loaded graph; current analyzer coverage may not expose application entrypoints.");
        }
    ...
    private static IReadOnlyDictionary<string, IReadOnlyList<GraphEdge>> BuildNonContainmentEdgesByNode(GraphDocument graph)
    {
        var edgesByNode = new Dictionary<string, List<GraphEdge>>(StringComparer.Ordinal);
