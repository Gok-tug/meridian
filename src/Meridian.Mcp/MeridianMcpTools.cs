...
public sealed class MeridianMcpTools
{
    ...
    [Description("Return graph metadata, available node kinds, available edge relations, and tool names. " + SharedNote)]
    ...
    [Description("Typed graph search; do not pass a custom DSL or natural-language question. Use JSON parameters such as text, nodeKind, relation, direction, source, target, and maxResults. " + SharedNote)]
    ...
    {
        return _service.QueryGraph(text, nodeKind, relation, direction, source, target, maxResults);
    }

    [McpServerTool(Name = "get_node", ReadOnly = true, Destructive = false, Idempotent = true, OpenWorld = false, UseStructuredContent = true)]
    [Description("Resolve one graph node by exact id, label, or symbol. Ambiguous queries return candidates instead of selecting silently. " + SharedNote)]
    ...
    [Description("Return bounded incoming, outgoing, or both-direction neighbors for one resolved node. Results are horizontally capped and include TRUNCATED guidance when the cap is reached. " + SharedNote)]
    public GraphSearchResponse GetNeighbors(
        ...
        [Description("Maximum returned nodes and edges. The server caps this to protect the agent context window.")] int? maxResults = null)
    {
        return _service.GetNeighbors(idOrLabel, direction, depth, relation, maxResults);
    }
    ...
    [Description("Find the shortest directed path between two resolved graph nodes using existing graph edges only. " + SharedNote)]
    ...
    [Description("Find and explain a directed graph path with relation, confidence, and optional evidence. " + SharedNote)]
    ...
    [Description("List emitted application entrypoint nodes. If empty, current analyzers do not yet emit ASP.NET endpoint nodes. " + SharedNote)]
    ...
    [Description("Reverse-traverse existing graph edges to find upstream nodes that can reach a target symbol. If no endpoint nodes exist, returns upstream nodes with an endpoint analyzer limitation. " + SharedNote)]
    ...
    {
        return _service.FindFlowsToSymbol(target, maxDepth, maxResults);
    }
}
