...
namespace Meridian.AnalyzerTests.MediatR;

public sealed class MediatRGoldenTests
{
    [Fact]
    public async Task MediatR_declaration_analyzer_matches_golden_graph()
    {
        var root = FindRepositoryRoot();
        var projectPath = Path.Combine(root, "samples", "Sample.MediatR", "Sample.MediatR.csproj");
        var expectedPath = Path.Combine(root, "tests", "Meridian.AnalyzerTests", "MediatR", "Expected", "MediatR.graph.json");
